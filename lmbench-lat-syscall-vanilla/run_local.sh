...
while [ -z "$BENCHMARK_PID" ]; do
        ...
        if [ $EXEC_TYPE -lt 3 ];then
            ...
            ps aux|grep ./bin/x86_64-Linux/lat_syscall|grep -v color|grep -v perf|grep -v "grep"
            ps aux|grep ./bin/x86_64-Linux/lat_syscall|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}'
            BENCHMARK_PID=$(ps aux|grep ./bin/x86_64-Linux/lat_syscall|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}')

        elif [ $EXEC_TYPE -eq 4 ];then
            echo "========"
            ps aux|grep lat_syscall|grep -v color|grep -v perf|grep -v "grep"
            echo "========"
            # ps aux|grep lat_syscall|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}'
            BENCHMARK_PID=$(ps aux|grep lat_syscall|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}')
        fi
        echo "========"
