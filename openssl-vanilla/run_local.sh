...
while [ -z "$BENCHMARK_PID" ]; do
        ...
        if [ $EXEC_TYPE -lt 3 ];then
            ...
            ps aux|grep ./encrypt|grep -v color|grep -v perf|grep -v "grep"
            ps aux|grep ./encrypt|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}'
            BENCHMARK_PID=$(ps aux|grep ./encrypt|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}')
