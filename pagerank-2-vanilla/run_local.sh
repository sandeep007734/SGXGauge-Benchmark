...
while [ -z "$BENCHMARK_PID" ]; do
        ...
        if [ $EXEC_TYPE -lt 3 ];then
            ...
            ps aux|grep ./bin/pagerank|grep -v color|grep -v perf|grep -v "grep"
            ps aux|grep ./bin/pagerank|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}'
            BENCHMARK_PID=$(ps aux|grep ./bin/pagerank|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}')
