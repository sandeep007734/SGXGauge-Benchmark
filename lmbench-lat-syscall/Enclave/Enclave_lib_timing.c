	...
	void* cookie)
{
	...
	// #ifdef _DEBUG_TIM
	snprintf(buf, BUFSIZE, "\tFILE: %s LINE:%d \tenough=%d",
	...
	if (_benchmp_child_state.r){
		...
		#ifdef _DEBUG_TIM
		snprintf(buf, BUFSIZE, "\tFILE: %s LINE:%d benchmp: r already allocated.\n",
				__FILE__, __LINE__);
	    ocall_print_string(buf);
    	memset(buf, '\0', BUFSIZE);
		#endif
		...
		#ifdef _DEBUG_TIM
		snprintf(buf, BUFSIZE, "\tFILE: %s LINE:%d benchmp: r not allocated.\n",
				__FILE__, __LINE__);
	    ocall_print_string(buf);
    	memset(buf, '\0', BUFSIZE);
		#endif
...
uint64 gettime(void)
{
	...
	// snprintf(buf, BUFSIZE, "\tFILE: %s:%d \t REACHED GETTIME",
	// 		__FILE__, __LINE__);
 //   			// ocall_print_string(buf);
 //    memset(buf, '\0', BUFSIZE);
