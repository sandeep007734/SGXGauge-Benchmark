...
void send_params(int parallel, int warmup, int repetitions){	
	...
	// wrapper_ecall_print_string(enclaveID, "987");

	ecall_send_params(parallel, warmup, repetitions);
	// wrapper_ecall_send_params(enclaveID, parallel, warmup, repetitions);

	// printf("FILE: %s LINE:%d destroy enclaveID: [%lu].\n",
 //        __FILE__, __LINE__, enclaveID);

	// printf("FILE: %s LINE:%d %s destroyed enclaveID: [%lu].\n",
	// 	__FILE__, __LINE__,
