	...
	void* cookie)
{
	...
	// #ifdef _DEBUG_TIM
	snprintf(buf, BUFSIZE, "\tAISHAPRINT FILE: %s LINE:%d \tenough=%d",
	...
	if (_benchmp_child_state.r){
		...
		#ifdef _DEBUG_TIM
		snprintf(buf, BUFSIZE, "%s:%d benchmp: r already allocated.\n",
				__FILE__, __LINE__);
	    ocall_print_string(buf);
    	memset(buf, '\0', BUFSIZE);
		#endif
		...
		#ifdef _DEBUG_TIM
		snprintf(buf, BUFSIZE, "%s:%d benchmp: r not allocated.\n",
				__FILE__, __LINE__);
	    ocall_print_string(buf);
    	memset(buf, '\0', BUFSIZE);
		#endif
