...
elif [ "$WORKLOAD_TYPE" = "6" ]; then
    BENCH_ARGS="-cASSIGN.DAT"
elif [ "$WORKLOAD_TYPE" = "7" ]; then
    BENCH_ARGS="-cIDEA.DAT"
elif [ "$WORKLOAD_TYPE" = "8" ]; then
    BENCH_ARGS="-cHUFF.DAT"
elif [ "$WORKLOAD_TYPE" = "9" ]; then
    BENCH_ARGS="-cNEURO.DAT"
elif [ "$WORKLOAD_TYPE" = "10" ]; then
    BENCH_ARGS="-cLU.DAT"
...
while [ -z "$BENCHMARK_PID" ]; do
        ...
        if [ $EXEC_TYPE -lt 3 ];then
            ...
            ps aux|grep ./nbench|grep -v color|grep -v perf|grep -v "grep"
            ps aux|grep ./nbench|grep -v perf|grep -v "grep"|awk '{print $2}'
            BENCHMARK_PID=$(ps aux|grep ./nbench|grep -v color|grep -v perf|grep -v "grep"|awk '{print $2}')
